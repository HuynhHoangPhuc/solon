...
### View Mode
- `/solon:preview <file.md>` - View markdown file in novel-reader UI
- `/solon:preview <directory/>` - Browse directory contents
- `/solon:preview --stop` - Stop running server

### Generation Mode
- `/solon:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/solon:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/solon:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/solon:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)
...
| Flag without topic | Ask user: "Please provide a topic: `/solon:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/solon:preview --stop` first |
