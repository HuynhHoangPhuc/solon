...
### Planning Phase
Activate **solon:plan** skill with mode-appropriate flag:
- `--full` → `/solon:plan --hard <requirements>` (thorough research + validation)
- `--auto` → `/solon:plan --auto <requirements>` (auto-detect complexity)
- `--fast` → `/solon:plan --fast <requirements>` (skip research)
- `--parallel` → `/solon:plan --parallel <requirements>` (file ownership + dependency graph)

Planning skill outputs a plan path. Pass this to cook.

### Implementation Phase
Activate **solon:cook** skill with the plan path and mode-appropriate flag:
- `--full` → `/solon:cook <plan-path>` (interactive review gates)
- `--auto` → `/solon:cook --auto <plan-path>` (skip review gates)
- `--fast` → `/solon:cook --auto <plan-path>` (skip review gates)
- `--parallel` → `/solon:cook --parallel <plan-path>` (multi-agent execution)
