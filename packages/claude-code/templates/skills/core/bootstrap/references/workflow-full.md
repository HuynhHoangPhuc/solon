...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `solon:ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `solon:ai-multimodal` skill
5. Screenshot wireframes with `solon:chrome-devtools` → save to `./docs/wireframes/`
...
Activate **solon:plan** skill: `/solon:plan --hard <requirements>`
...
Activate **solon:cook** skill: `/solon:cook <plan-path>` (interactive mode — review gates at each step)
