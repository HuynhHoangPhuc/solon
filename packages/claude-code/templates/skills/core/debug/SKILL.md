...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `solon:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `solon:docs-seeker` skill for package/plugin docs; `repomix` for codebase summary
- **Scouting:** `/solon:scout` or `/solon:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `solon:chrome-devtools` skill for visual verification (screenshots, console, network)
- **Skills:** Activate `solon:problem-solving` skill when stuck on complex issues
