...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `solon:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `solon:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `solon:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `solon:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
...
- Visual chesolon: [pass | issues found]
