...
- Use `/solon:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/solon:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/solon:plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/solon:plan validate` slash command
...
- Use `solon:ai-multimodal` for image assets
