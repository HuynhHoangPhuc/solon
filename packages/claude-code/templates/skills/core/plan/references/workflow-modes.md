...
4. **Context reminder:** `/solon:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/solon:scout` to search codebase
...
7. **Context reminder:** `/solon:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/solon:cook --parallel {absolute-plan-path}/plan.md`
...
```
Skill(skill: "solon:plan:red-team", args: "{plan-directory-path}")
```
...
| `auto` | Use the `Skill` tool: `Skill(skill: "solon:plan:validate", args: "{plan-directory-path}")` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool to invoke `plan:validate`:
```
Skill(skill: "solon:plan:validate", args: "{plan-directory-path}")
```
...
After creating: `node $HOME/.claude/scripts/set-active-plan.cjs {plan-dir}`
