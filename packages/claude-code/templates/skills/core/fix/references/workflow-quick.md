...
### Step 1: Debug
Activate `solon:debug` skill. Find root cause quickly. Verify the root cause with multiple `Explore` subagents in parallel.
...
Prompt: "Quick review of fix for [issue]. Chesolon: correctness, security, no regressions. Score X/10."
...
- `solon:debug` - Always (Step 1)
- Parallel `git-manager` - Verification (Step 2)
- `code-reviewer` subagent - Always (Step 3)
- `solon:context-engineering` - If dealing with AI/LLM code
