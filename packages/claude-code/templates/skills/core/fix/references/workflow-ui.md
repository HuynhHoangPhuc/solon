...
1. `solon:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `solon:ui-ux-pro-max` - Design principles
3. `solon:frontend-design` - Implementation patterns
...
Analyze screenshots/videos with `solon:ai-multimodal` skill.
...
Screenshot + `solon:ai-multimodal` analysis.
...
Use `solon:chrome-devtools` skill.
...
## Tips
- Use `solon:ai-multimodal` for generating visual assets
