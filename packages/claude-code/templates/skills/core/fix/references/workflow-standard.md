...
Activate `solon:debug` skill. Use `debugger` subagent if needed.
...
- Apply `solon:problem-solving` skill if stuck
- Use `solon:sequential-thinking` for complex logic
...
|------|------------------|
| 1 | `solon:debug`, `debugger` subagent |
| 2 | Multiple `Explore` subagents in parallel (optional) |
| 3 | `solon:problem-solving`, `solon:sequential-thinking`, parallel `Bash` for verification |
| 4 | `tester` subagent |
