...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `$HOME/.claude/.solon.json` (or `~/.claude/.solon.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `$HOME/.claude/.solon.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `solon:docs-seeker` skill to find read it.
