...
Read model from `$HOME/.claude/.solon.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `$HOME/.claude/.mcp.json`:
...
ln -sf $HOME/.claude/.mcp.json .gemini/settings.json
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `$HOME/.claude/.mcp.json`)
...
Should show symlink pointing to `$HOME/.claude/.mcp.json`.
