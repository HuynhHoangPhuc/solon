...
```bash
cd $HOME/.claude/skills/mcp-management/scripts
npm install
```

### 2. Configure MCP Servers

Create `$HOME/.claude/.mcp.json`:
...
See `$HOME/.claude/.mcp.json.example` for more examples.
...
- Load config from `$HOME/.claude/.mcp.json`
...
2. `$HOME/.claude/skills/mcp-management/.env`
...
The `mcp-manager` agent (`$HOME/.claude/agents/mcp-manager.md`) uses this skill to:
...
Ensure `$HOME/.claude/.mcp.json` exists and is valid JSON.
