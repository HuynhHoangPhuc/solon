...
Entry point for using the `solon:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `solon:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `solon:frontend-design` with `solon:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `solon:chrome-devtools` Skill
Use `solon:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `solon:media-processing` Skill
