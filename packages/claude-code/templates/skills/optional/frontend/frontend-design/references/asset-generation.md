...
Complete workflow for generating design-aligned visual assets using `solon:ai-multimodal` skill.
...
Use `solon:ai-multimodal` skill's image generation capabilities:
...
Use `solon:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `solon:media-processing` skill for post-generation adjustments
