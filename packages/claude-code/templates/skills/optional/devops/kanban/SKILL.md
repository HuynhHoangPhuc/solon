...
- `/solon:kanban` - View dashboard for ./plans directory
- `/solon:kanban <dir>` - View dashboard for specific directory
- `/solon:kanban --stop` - Stop running server
...
The skill is located at `$HOME/.claude/skills/plans-kanban/`.
...
```json
{
  "command": "node $HOME/.claude/skills/plans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
...
The `/solon:kanban` command will evolve into **VibeKanban-inspired** AI agent orchestration:
