...
- **Sync-basolon:** Reconcile all completed tasks against all phase files, update `[ ]` → `[x]`, update YAML frontmatter status
...
- `solon:plan` — Creates implementation plans (planning phase)
- `solon:cook` — Implements plans (execution phase, invokes project-manager at finalize)
