...
describe("generateSettings", () => {
  ...
  it("preserves existing hooks when merging", () => {
    const existing = {
      hooks: {
        PreToolUse: [
          {
            matcher: "Edit",
            hooks: [{ type: "command" as const, command: "my-hook" }],
          },
        ],
      },
    };
    ...
    const hasExistingHook = preToolUse.some((b) =>
      b.hooks?.some((h) => h.command === "my-hook"),
    );
  ...
  it("includes hashline-edit hook on PreToolUse Edit matcher", () => {
    ...
    const hashlineHook = preToolUse.find(
      (b) =>
        b.matcher === "Edit" &&
        b.hooks?.some((h) => h.command?.includes("hashline-edit")),
    );
