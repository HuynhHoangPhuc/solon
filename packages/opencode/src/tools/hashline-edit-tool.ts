...
export const HashlineEditInputSchema = z.object({
  ...
  edits: z.array(EditOperationSchema).describe('Edit operations using LINE#ID refs'),
});
...
export async function hashlineEditHandler(input: HashlineEditInput): Promise<string> {
  ...
  for (const edit of input.edits) {
    const posLine = edit.pos ? parseInt(edit.pos.split('#')[0] ?? '0', 10) : 0;
    const endLine = edit.end ? parseInt(edit.end.split('#')[0] ?? '0', 10) : undefined;
    const newLines = edit.lines == null
      ? []
      : Array.isArray(edit.lines)
        ? edit.lines
        : [edit.lines];
    resolved.push({ op: edit.op, lineNumber: posLine, endLineNumber: endLine, newLines });
  }
  ...
  for (const op of resolved) {
    ...
    if (op.op === 'replace') {
      ...
      fileLines.splice(op.lineNumber > 0 ? idx + 1 : fileLines.length, 0, ...op.newLines);
      editCount++;
