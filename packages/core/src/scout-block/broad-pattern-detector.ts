...
const BROAD_PATTERNS: Array<{ regex: RegExp; suggestion: string }> = [
  ...
  { regex: /^\*\*\/\*\.\w+$/, suggestion: 'Prefix with a directory, e.g. src/**/*.ts' },
  { regex: /^\*$/, suggestion: 'Use a specific directory glob like src/*' },
  { regex: /^\.\/\*$/, suggestion: 'Use ./{dir}/* to target a specific folder' },
 ...
 */
export function checkBroadPattern(pattern: string, _toolName: string): BroadPatternResult {
