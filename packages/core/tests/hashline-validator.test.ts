...
import { validateLineRefs, HashlineMismatchError } from '../src/hashline/hash-line-validator.js';
...
describe('validateLineRefs', () => {
  ...
  it('throws HashlineMismatchError for stale ref', async () => {
    ...
    await expect(validateLineRefs([staleRef], lines)).rejects.toThrow(HashlineMismatchError);
  });
  ...
  it('throws for out-of-bounds line number', async () => {
    await expect(validateLineRefs(['999#ZZ'], lines)).rejects.toThrow('out of bounds');
  });
});
