...
describe('truncateOutput', () => {
  ...
  it('preserves header lines at top', () => {
    ...
    const body = Array.from({ length: 200 }, (_, i) => `body line ${i}: ${'y'.repeat(30)}`);
  ...
  it('appends truncation notice with count', () => {
    ...
    expect(result).toMatch(/\[\d+ more lines truncated due to context window limit\]/);
  });
  ...
  it('uses default 3 header lines when not specified', () => {
    const lines = Array.from({ length: 200 }, (_, i) => `x line ${i}: ${'a'.repeat(30)}`);
    const text = lines.join('\n');
