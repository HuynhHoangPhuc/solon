...
describe('isBlockedDirectory', () => {
  ...
  it('respects custom patterns', () => {
    expect(isBlockedDirectory('/project/custom-blocked/file.ts', ['custom-blocked'])).toBe(true);
  });
});
...
describe('checkScoutBlock', () => {
  ...
  it('blocks node_modules path in tool input', async () => {
    const result = await checkScoutBlock('Read', { file_path: '/project/node_modules/foo.js' });
    expect(result.blocked).toBe(true);
  ...
  it('allows normal file path', async () => {
    const result = await checkScoutBlock('Read', { file_path: '/project/src/index.ts' });
    expect(result.blocked).toBe(false);
  });
