...
func ReadSessionState(sessionID string) *hookio.SessionState {
	if sessionID == "" {
		return nil
	}
...
func WriteSessionState(sessionID string, state *hookio.SessionState) bool {
	if sessionID == "" || state == nil {
		return false
	}
	target := GetSessionTempPath(sessionID)
	tmp := fmt.Sprintf("%s.%s", target, randomSuffix())
