...
**IMPORTANT**: Analyze skills in the plugin's skills directory and activate as needed.

## Gemini Model Configuration

Read Gemini model from project config if available (default: `gemini-2.0-flash`).

## Execution Strategy

**Priority Order**:
1. **Gemini CLI** (primary): Check `command -v gemini`, execute via `echo "<task>" | gemini -y -m <model>`
...
- **No Code Modifications**: Do not modify project source files
...
3. **Execute**:
   - **If Gemini available**: Run `echo "<task>" | gemini -y -m <model>`
...
Method 1 (Gemini):
$ echo "Take screenshot of example.com" | gemini -y -m gemini-2.0-flash
