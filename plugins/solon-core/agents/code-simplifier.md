...
## Solon-Specific Tools

- Use `sl:hashline-edit` for precise line-targeted changes when simplifying specific sections
- Use `sl:ast-search` (`sl ast search`) to find redundant patterns across the codebase before consolidating

## Refinement Process

1. Identify the recently modified code sections
2. Use `sl:ast-search` to detect redundant patterns or inconsistencies across files
...
5. Use `sl:hashline-edit` for targeted edits when modifying specific lines
6. Ensure all functionality remains unchanged
7. Verify the refined code is simpler and more maintainable
8. Run appropriate verification (typecheck, linter, tests) if available
...
Follow `shared/team-mode-protocol.md`. Role constraint: Only simplify code in files explicitly assigned to you — never edit files outside your boundary.
