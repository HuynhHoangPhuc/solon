...
These phases apply after planning is complete and ship skill is activated.
Ship skill handles most of these — this reference documents bootstrap-specific guidance.

## Implementation

Handled by **sl:ship** skill. Bootstrap-specific notes:
...
- Asset pipeline: `sl:ai-multimodal` (generate/analyze) → `imagemagick` (crop/resize) → background removal if needed
