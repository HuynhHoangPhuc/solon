...
1. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `sl:ai-multimodal` skill generation
2. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
3. If no logo provided: generate with `sl:ai-multimodal` skill
4. Screenshot wireframes with browser DevTools → save to `./docs/wireframes/`
...
Activate **sl:plan** skill: `/sl:plan --auto <requirements>`
...
Activate **sl:ship** skill: `/sl:ship --auto <plan-path>`
