...
Activate `sl:debug` skill. Use `debugger` subagent if needed.
...
- Apply `sl:problem-solving` skill if stuck
- Use `sl:sequential-thinking` for complex logic
...
|------|------------------|
| 1 | `sl:debug`, `debugger` subagent |
| 2 | Multiple `Explore` subagents in parallel (optional) |
| 3 | `sl:problem-solving`, `sl:sequential-thinking`, parallel `Bash` for verification |
| 4 | `tester` subagent |
| 5 | `code-reviewer` subagent |
| 6 | `git-manager`, `docs-manager` subagents |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `chrome`, `sl:chrome-devtools` or any relevant skills/tools to verify.
